      ...
      <Paper style={{ marginTop: '3%', marginLeft: '2%', width: '95%' }} lg={10} >
        <Grid container style={{ marginLeft: '2%', marginTop: '2%' }} lg={10}>
          ...
          <Grid item style={{ marginTop: '10%' }} lg={1}>
            <Button color='secondary' size='large' variant='contained' onClick={() => this.props.handleDetails(this.props.row)}>Details</Button>
          </Grid>
        </Grid>
      </Paper>
    )
...
class ImgMediaCard extends React.Component {
  ...
  render() {
    ...
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, this.props.listings.length - page * rowsPerPage);
      ...
      <Paper className={classes.root} style={{ width: '97%', marginLeft: '2%', marginBottom: '2%' }} >
        <div className={classes.tableWrapper}>
          <Table className={classes.table}>
            <TableBody>
              {this.props.listings.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                return (
                  <SmallPaper row={row} handleDetails={this.props.handleDetails} cardProps={classes.card} imgC={h2} houseName={row.Address} properySize={5} bedroom={row.Bedrooms} changePAge={this.props.changePAge} bathrooms={row.Bathrooms} rent={row.rent} />
                );
              })}
              {emptyRows > 0 && (
