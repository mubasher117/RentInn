...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleUser: (id) => {
      dispatch(SellerServer.handleUser(id));
    },
    handleOrder: (
      ownerid,
      lat,
...
class SellerView extends Component {
  ...
  getAvatar() {
    if (
      ...
      return (
        <Avatar className={this.props.classes.purpleAvatar}>
          {this.props.user[0].name[0].toUpperCase()}
        </Avatar>
      );
    }
  }
  getName() {
    if (
      ...
      return (
        <Typography style={{ color: "white", marginRight: "1%" }}>
          {this.props.user[0].name.toUpperCase()}
        </Typography>
      );
    }
  }
  getSignOutButton() {
    if (this.props.match.params.userId !== undefined) {
      return (
        <Button onClick={this.logout} variant="raised" color="secondary">
          Log Out{" "}
        </Button>
      );
    } else {
  ...
  getScreen(status) {
    ...
    switch (status) {
      case seller_Status.seller_SignIn.NEW:
        return (
          <SellerChild
            isLogged={this.props.match.params.userId}
      ...
      case seller_Status.seller_SignIn.SUCCESS:
        return <SellerSucces handleBack={this.handleBackClick} />;
        break;
    }
  }
  logout() {
...
export default withStyles(styles)(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(SellerView))
);
